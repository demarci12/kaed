...
export const POST: APIRoute = async ({ request, cookies, redirect, params }) => {
	...
	const { data: actor } = await supabase.from('system_actors').select('project_id').eq('id', params.id).maybeSingle();
	const { error } = await supabase.from('system_actors').delete().eq('id', params.id);
