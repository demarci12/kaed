...
const EDITABLE_FIELDS = new Set(['title', 'body', 'evidence', 'kind']);
const KINDS = new Set(['obsession', 'skill', 'experience', 'strength']);
...
export const POST: APIRoute = async ({ request, cookies, params }) => {
	...
	if (field === 'kind' && value && !KINDS.has(value)) {
	...
	const { error } = await supabase.from('knowledge_cards').update(update).eq('id', params.id);
